...
// Handle GET request for partner count
export async function GET(request: Request) {
  console.log('Fetching partner count...');
  try {
    const client = await pool.connect();

    // Query to count total partners, those added today, yesterday, in the last 7 days, and in the last 30 days
    const query = `
      SELECT 
    ...
    return NextResponse.json(
      { message: 'Error fetching partner count', error: (error as Error).message },
      { status: 500 }
    );
  }
}
