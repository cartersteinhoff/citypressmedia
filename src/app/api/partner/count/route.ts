...
export async function GET(request: Request) {
  ...
  try {
    ...
    const result = await client.query('SELECT COUNT(*) AS count FROM partners');
    client.release();

    const partnerCount = result.rows[0].count;
    console.log('Chapter leader count fetched:', partnerCount);

    return NextResponse.json({ partnerCount });
  } catch (error) {
